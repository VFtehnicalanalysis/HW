...
def download_index_data(indexes, start_date, end_date):
    with requests.Session() as session:
...
"""
def download_repo_data(ticker, start_date, end_date):
    with requests.Session() as session:
        request_params = {
            'interval': 24,  # Дневные свечи
            'start': start_date,
...
download_and_save_index_data(ticker, start_date, end_date)
"""